...
if __name__ == '__main__':
	#========================【 输 入 】===========================
	module,module_name,module_unit = input_.select_module() #模块选择 
	aspect,aspect_name,unite = input_.select_aspect() #分析内容选择
	dateChoose = input_.select_datechoose() #时间选项选择
	sql_date1,sql_date2 = input_.select_date() #时间选择


	if module == 4:
		sql_cust = input_.select_cust() #客户选择
		if module_unit == "时间":
			tradeNo_list,tradeNoList = input_.select_trade() #钢种选择
		elif module_unit == "钢种":
			pass
		else:
			pass
		...
		tradeNo_list,tradeNoList = input_.select_trade() #钢种选择
		space,sql_ctry_prov_cty,space_name,space_dict = input_.select_space(module) #地点选择
	...
	if module == 1:  #空间分析	#前两个的 max_min_ave_sum 与 final_conclusion 是一样的
		...
		# 【 结 论 输 出 】
		conclusion = conclusion.final_conclusion(sql_date1,sql_date2,tradeNoList,space_name,aspect_name,printMax,maxRateReason,sumValue,unite,averageValue,maxKey,maxValue,minKey,minValue,maxRate100,aspect,passOrNot,tradeNo_rtn_reason_print,module_unit,noMin,module)
		...
		# 【 结 论 输 出 】 #与前两个不一样
		conclusion = conclusion.final_conclusion_trade(sql_date1,sql_date2,tradeNoList,space_name,aspect_name,printMax,maxRateReason,sumValue,unite,averageValue,maxKey,maxValue,minKey,minValue,maxRate100,aspect,passOrNot,tradeNo_rtn_reason_print,module_unit,noMin,tradeNo_rate_dict,module)
		...
		if module_unit == "时间":
			...
			maxValue,maxKey,minValue,minKey,noMin,sumValue,averageValue,printMax,maxRate,maxRate100,maxRateReason = max_min_average.max_min_ave_sum(dictionary,aspect,module_unit)
			# 【 结 论 输 出 】
			conclusion = conclusion.final_conclusion_cust(sql_date1,sql_date2,tradeNoList,sql_cust,aspect_name,printMax,maxRateReason,sumValue,unite,averageValue,maxKey,maxValue,minKey,minValue,maxRate100,aspect,passOrNot,tradeNo_rtn_reason_print,module_unit,noMin,module)
		elif module_unit == "钢种":
			...
			maxValue,maxKey,minValue,minKey,noMin,sumValue,averageValue,printMax,maxRate,maxRate100,maxRateReason = max_min_average.max_min_ave_sum(dictionary,aspect,module_unit)
			# 【 结 论 输 出 】
			conclusion = conclusion.final_conclusion_cust2(sql_date1,sql_date2,sql_cust,aspect_name,printMax,maxRateReason,sumValue,unite,averageValue,maxKey,maxValue,minKey,minValue,maxRate100,aspect,passOrNot,tradeNo_rtn_reason_print,module_unit,noMin,module)
		else:
			pass
