...
def regression_ana(result):
    """
    read relation analyse data of output and control elements.
    """
    ...
    db_table_names = {
        '32':"regression_cof_output_middle",
        '31':"regression_cof_output_input",
        '21':"regression_cof_middle_input"
    }
    entrance = [(3,2), (3, 1), (2, 1)]
    for variables in entrance:
        DEPENDENT_VARIABLE_ATTRIBUTE_NUMBER, INDEPENDENT_VARIABLE_ATTRIBUTE_NUMBER = variables
        ...
        elesdf = pd.DataFrame(rows)
        cols = list(elesdf.columns)

        #convert strnumber to abs value.
        elesdf[cols[2]] = elesdf[cols[2]].map(lambda x: abs(float(x)))
        #get all output eles.
        outputs = list(elesdf[cols[0]].unique())

        # #create database connect use default config in file 'config.py'.
        # db_conn = oracle.Oracle()
        # #create table by given tablename
        # sql = 'drop table QG_USER.%s' % db_table_name
        # db_conn.execute(sql)
        # sql = 'create table QG_USER.%s(outfield varchar(50),midinfield varchar(50),cof varchar(50))' % db_table_name
        # db_conn.execute(sql)

        with open(db_table_name + '.txt','w') as f:
            for output in outputs:

                eles_singleout = elesdf[elesdf[cols[0]] == output]

                #sort by absolute r_value.
                eles_singleout_sorted_de = eles_singleout.sort_values(by=cols[2],ascending=False)

                #list all reserved eles by given reserve percent.
                length_elses = len(eles_singleout)
                reserveNum = math.ceil(length_elses * ( 1 - percentNum))

                selected_elesdf = eles_singleout_sorted_de[:reserveNum]
                selected_eles = list(selected_elesdf[cols[1]])
                sql = 'SELECT ' + ', '.join(selected_eles) + ', ' + output + ' from QG_USER.PRO_BOF_HIS_ALLFIELDS'
                db_name = 'l2own'
                sqlVO = dict()
                sqlVO['sql'] = sql
                ...
                #     sql = 'insert into QG_USER.%s values(\'%s\', \'%s\', \'%s\')'%(db_table_name, output, selected_eles[i], coef[i])
                #     db_conn.execute(sql)
                #     f.write(', '.join(['%s ']*3)%(output, selected_eles[i], coef[i]))
                #     f.write('\n')
                # sql = 'insert into QG_USER.%s values(\'%s\', \'BIAS\', \'%s\')'%(db_table_name, output, str(intercept))
                # db_conn.execute(sql)
